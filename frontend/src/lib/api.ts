...
export interface BookDetail {
  book: BookItem;
  units: { index: number; title: string; lessons: BookLesson[] }[];
}
...
export interface LessonContent {
  ...
  glossary: { term?: string; definition?: string; [k: string]: unknown }[];
  created_at?: string;
  updated_at?: string;
}
...
export interface GenerateResult {
  generated: number[];
  failed: { lesson_id?: number; error?: string }[];
}
...
function providerHeaders(settings?: ProviderSettings): Record<string, string> {
  const h: Record<string, string> = { "Content-Type": "application/json" };
  if (settings?.apiKey) h["x-provider-key"] = settings.apiKey;
  return h;
}
...
export async function fetchBooks(): Promise<BookItem[]> {
  ...
  if (!res.ok) throw new Error(await readError(res, "فشل تحميل الكتب"));
  return res.json();
}

export async function fetchBook(id: number): Promise<BookDetail> {
  const res = await fetch(`/api/books/${id}`);
  if (!res.ok) throw new Error(await readError(res, "فشل تحميل الكتاب"));
  return res.json();
}
...
export async function uploadBook(file: File): Promise<BookItem> {
  ...
  if (!res.ok) throw new Error(await readError(res, "فشل رفع الكتاب"));
  return res.json();
}

export async function deleteBook(id: number): Promise<void> {
  const res = await fetch(`/api/books/${id}`, { method: "DELETE" });
  if (!res.ok) throw new Error(await readError(res, "فشل حذف الكتاب"));
}

export async function analyzeBook(id: number): Promise<BookLesson[]> {
  const res = await fetch(`/api/books/${id}/analyze`, { method: "POST" });
  if (!res.ok) throw new Error(await readError(res, "فشل تحليل الكتاب"));
  return res.json();
}
...
export async function generateBookLessons(
  ...
  if (!res.ok) throw new Error(await readError(res, "فشل توليد الدروس"));
  return res.json();
}

export async function fetchLesson(lessonId: number): Promise<LessonContent> {
  const res = await fetch(`/api/books/lessons/${lessonId}`);
  if (!res.ok) throw new Error(await readError(res, "فشل تحميل الدرس"));
  return res.json();
}
...
export async function regenerateLesson(
  ...
  if (!res.ok) throw new Error(await readError(res, "فشل إعادة التوليد"));
  return res.json();
}
...
export async function searchLessons(
  q: string,
  bookId?: number,
): Promise<{ id: number; book_id: number; title: string; unit_title: string; snippet: string }[]> {
  const params = new URLSearchParams({ q });
  if (bookId != null) params.set("book_id", String(bookId));
  const res = await fetch(`/api/books/search?${params}`);
  if (!res.ok) throw new Error(await readError(res, "فشل البحث"));
  return res.json();
}
...
export async function exportQuizJson(
  ...
  const params = bookId != null ? `?book_id=${bookId}` : "";
  const res = await fetch(`/api/books/quiz/export${params}`);
  if (!res.ok) throw new Error(await readError(res, "فشل تصدير الأسئلة"));
  return res.json();
}
...
export async function importQuizJson(
  ...
  if (!res.ok) throw new Error(await readError(res, "فشل استيراد الأسئلة"));
  return res.json();
}

export async function fetchQuiz(bookId: number, lessonId?: number): Promise<QuizBankItem[]> {
  const params = lessonId != null ? `?lesson_id=${lessonId}` : "";
  const res = await fetch(`/api/books/${bookId}/quiz${params}`);
  if (!res.ok) throw new Error(await readError(res, "فشل تحميل بنك الأسئلة"));
  return res.json();
}
...
export async function submitQuizAttempt(
  quizId: number,
  answer: string,
): Promise<{ quiz_id: number; is_correct: boolean }> {
  const res = await fetch(`/api/books/quiz/${quizId}/attempt`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ answer }),
  });
  if (!res.ok) throw new Error(await readError(res, "فشل تسجيل الإجابة"));
  return res.json();
}
